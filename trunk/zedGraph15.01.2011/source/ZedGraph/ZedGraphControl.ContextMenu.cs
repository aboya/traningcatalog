...
using ZedGraph.zForms;
//using ZedGraph.ZedGraph.Forms;
...
namespace ZedGraph
{
	...
	#region ContextMenu
		...
		private void contextMenuStrip1_Opening( object sender, CancelEventArgs e )
		{
			...
			if ( _masterPane != null && menuStrip != null )
			{
				...
				if ( _isShowContextMenu )
				{
                    ...
                    item = new ToolStripMenuItem();
                    item.Name = "show_options";
                    item.Tag = "show_options";
                    menuStr = "Options";//_resourceManager.GetString("set_default");
                    item.Text = menuStr;
                    item.Click += new EventHandler(this.MenuClick_ShowOptions);
                    if (pane == null)
                        item.Enabled = false;
                    menuStrip.Items.Add(item);
		...
		*/
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void MenuClick_ShowOptions(System.Object sender, System.EventArgs e)
        {
            if (_masterPane != null)
            {
                GraphPane pane = _masterPane.FindPane(_menuClickPt);
                new GraphOptions(pane).ShowDialog();
                this.Refresh();
